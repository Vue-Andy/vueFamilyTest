<template>
  <div class="helloChina">
    <div v-title>I am helloChina component</div>
    <div>{{msg}}</div>
    <hr />
    <div>
    ...
    <p>
      <label for="inp">父组件</label>
      <input type="text" v-model="txt" @change="change" id="inp"/>
    </p>
    <child ref="c1"></child>
  </div>
</template>
...
export default {
  ...
  mounted() {
    var a = this.$route.query.cardId,
      b = this.$route.params.cardId,
      c = this.$router.currentRoute.query.cardId,
      d = this.$router.currentRoute.params.cardId;
    if (a) {
